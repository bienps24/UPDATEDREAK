...
const bot = new Telegraf(process.env.BOT5_TOKEN);
    ...
    try {
        ...
        try {
            ...
            if (err.response?.error_code === 429) {
                ...
                console.warn(`⏳ Bot 5 rate-limited. Retrying in ${wait}s...`);
                await new Promise(res => setTimeout(res, wait * 1000));
            } else {
                console.error("❌ Bot 5 reaction error:", err);
            }
        }

    } catch (err) {
        console.error("❌ Bot 5 error:", err);
    }
});

bot.command('status', (ctx) => {
    if (!ADMIN_IDS.includes(ctx.from.id.toString())) return;
    ctx.reply("✅ Bot 5 is online with anti-spam + rate-limit handling.");
});

bot.launch();
console.log("✅ Bot 5 is running with anti-spam + rate-limit protection...");
