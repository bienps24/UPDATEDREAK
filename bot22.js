...
const bot = new Telegraf(process.env.BOT22_TOKEN);
    ...
    try {
        ...
        try {
            ...
            if (err.response?.error_code === 429) {
                ...
                console.warn(`⏳ Bot 22 rate-limited. Retrying in ${wait}s...`);
                await new Promise(res => setTimeout(res, wait * 1000));
            } else {
                console.error("❌ Bot 22 reaction error:", err);
            }
        }

    } catch (err) {
        console.error("❌ Bot 22 error:", err);
    }
});

bot.command('status', (ctx) => {
    if (!ADMIN_IDS.includes(ctx.from.id.toString())) return;
    ctx.reply("✅ Bot 22 is online with anti-spam + rate-limit handling.");
});

bot.launch();
console.log("✅ Bot 22 is running with anti-spam + rate-limit protection...");
