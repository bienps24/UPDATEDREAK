...
const bot = new Telegraf(process.env.BOT2_TOKEN);
    ...
    try {
        ...
        try {
            ...
            if (err.response?.error_code === 429) {
                ...
                console.warn(`⏳ Bot 2 rate-limited. Retrying in ${wait}s...`);
                await new Promise(res => setTimeout(res, wait * 1000));
            } else {
                console.error("❌ Bot 2 reaction error:", err);
            }
        }
...
bot.command('status', (ctx) => {
    ...
    ctx.reply("✅ Bot 2 is online with anti-spam + rate-limit handling.");
});

bot.launch();
console.log("✅ Bot 2 is running with anti-spam + rate-limit protection...");
