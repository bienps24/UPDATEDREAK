    ...
    try {
        ...
        try {
            ...
            if (err.response?.error_code === 429) {
                ...
                console.warn(`⏳ Bot 1 rate-limited. Retrying in ${wait}s...`);
                await new Promise(res => setTimeout(res, wait * 1000));
            } else {
                console.error("❌ Bot 1 reaction error:", err);
            }
        }
...
bot.command('status', (ctx) => {
    ...
    ctx.reply("✅ Bot 1 is online with anti-spam + rate-limit handling.");
});

bot.launch();
console.log("✅ Bot 1 is running with anti-spam + rate-limit protection...");
