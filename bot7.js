    ...
    try {
        ...
        try {
            ...
            if (err.response?.error_code === 429) {
                ...
                console.warn(`⏳ Bot 7 rate-limited. Retrying in ${wait}s...`);
                await new Promise(res => setTimeout(res, wait * 1000));
            } else {
                console.error("❌ Bot 7 reaction error:", err);
            }
        }

    } catch (err) {
        console.error("❌ Bot 7 error:", err);
    }
});

bot.command('status', (ctx) => {
    if (!ADMIN_IDS.includes(ctx.from.id.toString())) return;
    ctx.reply("✅ Bot 7 is online with anti-spam + rate-limit handling.");
});

bot.launch();
console.log("✅ Bot 7 is running with anti-spam + rate-limit protection...");
